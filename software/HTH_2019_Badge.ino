...
//  for (row=0; row<h; row++) {
//        for (col=0; col<w; col++) { 
//          mydisp.drawPixel(col, row, pgm_read_word(image_data_hth + buffidx));
//          buffidx++;
//        } 
//   }
//   delay(2000);
...
{ 
  ...
  if (clickedItem > 0)
  {
    ...
    if (menu.CurrentMenu==mnuRoot)
      ...
      {
        case 1:
          menu.InitMenu((const char ** )mnuLora,cntLora,1);
          break;
        case 2:
...
//          }
     ...
     else if (menu.CurrentMenu==mnuLora)
      switch (clickedItem)
      {
        case 1:
...
//      { menu.InitMenu((const char ** )mnuRoot,cntRoot,6); }
      ...
      { menu.InitMenu((const char ** )mnuRoot,cntRoot,11); }
  ...
  //Send reeived LoRa message back to client via websocket
  if(msgReceived == true){
     ws.textAll(storedMsg);
     msgReceived = false;
  }
}
